...
async function getEvents(query) {
	...
	// Limit by date
	let from = query.start ? new Date(query.start) : undefined;
	let to = query.end ? new Date(query.end) : undefined;
	...
	if (hasSearchFilter(query)) {
		...
		do {
			...
			const pageEvents = await events.getEvents(EVENT_TYPE, pageStart, pageStop, from || '-inf', to);
...
async function hideExistingUids(events) {
	...
	return events.filter((event, idx) => !isMembers[idx]);
}
