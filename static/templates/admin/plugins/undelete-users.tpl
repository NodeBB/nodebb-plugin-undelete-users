<div class="row events undelete-users">
	...
	<div class="col-lg-3 acp-sidebar">
		<div class="card">
			<div class="card-header">{{tx("admin/advanced/events:filters")}}</div>
			<div class="card-body">
				<form role="form" id="filters">
					<div class="mb-3">
						<label class="form-label" for="username">{{tx("admin/manage/users:search.username")}}</label>
						<input type="text" id="username" name="username" value="{query.username}" class="form-control" />
					</div>
					<div class="mb-3">
						<label class="form-label" for="uid">{{tx("admin/manage/users:search.uid")}}</label>
						<input type="text" id="uid" name="uid" value="{query.uid}" class="form-control" />
					</div>
					<div class="mb-3">
						<label class="form-label" for="email">{{tx("admin/manage/users:search.email")}}</label>
					...
					<button type="submit" id="filter" class="btn btn-primary w-100">{{tx("admin/advanced/events:filters-apply")}}</button>
				</form>
			</div>
		</div>
	</div>
